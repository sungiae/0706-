 ...
 *      오라클 : 공유된 데이터를 저장하는 곳 (영구적)
 *      자바 : 오라클 연결해서 데이터를 가지고 온 후에 => 브라우저로 전송
 *      HTML/CSS => 브라우저에 출력
 *      JavaScript => 실시간 전송 (AJAX) => 댓글, 날씨... 브라우저에서 이벤트 처리 (버튼, 메뉴...)
 *      ==============================================================================
 *      
 *      1. 자바
 ...
 *          기타 => 외부 라이브러리 (스프링, 마이바티스, Jsoup, simple-json...: mvnrepository.com)
 *          
 ...
 *          3. 수학 : Math
 *          4. 입출력 : System => 화면 출력, 입력값 받기, GC(가비지 컬렉션)(메모리 해제)
 *          5. Wrapper 클래스
 ...
 *                  => 다른 패키지에서 사용 : 상속, 재정의(오버라이딩) = 접근지정어를 확장해서 사용이 가능
 *                     ======== 사용이 가능 (모든 Object 상속)
 ...
 *            ex. Object[] o={10, 'A', 10.5};
 *                Object[] cls={new A() , new B() , new C() , new D()} 
 ...
 *                => 인터페이스 : 서로 다른 클래스를 여러개 묶어서 관리 ==> 웹 (요청을 받아서 응답하는 프로그램)
 *                
...
class Sawon
{
	...
	// 오른쪽클릭 - source - override- finalize
	// Sawon객체가 메모리에서 사라질 때 자동 호출되는 메소드(소멸자함수) => A(), ~A()
	@Override
	protected void finalize() throws Throwable { // finalize = 소멸자
